...
async function mergedPageText(style?: 'corner' | 'footer'): Promise<string> {
  const enclosure = { number: 1, title: 'Wiring Diagram', data: await onePagePdf() } as unknown as EnclosureData;
  const result = await mergeEnclosures(await onePagePdf(), [enclosure], undefined, false, [], style ? { enclosureLabel: style } : {});
