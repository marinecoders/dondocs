  ...
  describe(label, () => {
    ...
    it('tells a host that renders pages the letter is in the chat, and any other to present the file', async () => {
      ...
      try {
        ...
        // The editor is configured here, so the card sentence names that button too.
        expect(text(card)).toMatch(/\nThe letter is shown in the chat as a card, with an Edit in browser button .*out "host\.pdf"/);
