    ...
    VitePWA({
      ...
      workbox: {
        ...
        runtimeCaching: [
          ...
          {
            // Cache same-origin pandoc assets (pandoc.js, pandoc.wasm ~58 MB,
            // wasi-shim.js) for offline DOCX generation. These are vendored
            // into public/lib/pandoc/ (pandoc.wasm at build time by
            // scripts/vendor-assets.mjs) and served same-origin — no CDN.
            // CacheFirst so the 58 MB binary survives across sessions after the
            // first fetch; on a self-hosted air-gapped deployment the same-origin
            // fetch also works directly from the local server.
            urlPattern: /\/lib\/pandoc\/.*/,
            handler: 'CacheFirst',
            options: {
              cacheName: 'pandoc-wasm-cache-v2', // v2: same-origin only (was CDN)
              expiration: {
                maxEntries: 10,
          ...
          // NOTE: the previous unpkg.com / cdn.jsdelivr.net runtime-cache rules
          // for the pandoc WASM and WASI shim were removed. Those assets are now
          // vendored same-origin (see scripts/vendor-assets.mjs and the
          // air-gap fix in public/lib/pandoc/pandoc.js), so dondocs contacts no
          // third-party CDN at runtime. A CI guard (scripts/check-no-cdn.mjs)
          // fails the build if any CDN host literal reappears in dist/.
        ],
      },
    }),
  ],
