// Mirror GitHub issues and pull requests into the project's task tracker.
//
// One row per issue/PR, keyed on the tracker's GitHub URL column. Only the
// number, title, state, draft flag, and labels cross over — never the body.
// Issue bodies are where people paste documents, and the tracker rule is
// "names, units, and what we want from people; nothing from any document".
//
// The tracker is any HTTP API with a queryable Tasks collection; its base
// URL, extra headers, collection id and token all come from the workflow
// environment so nothing about it lives in the repo.
//
// Runs from the tracker-sync workflow: on issue/PR events it syncs that one
// item; on workflow_dispatch it backfills everything currently open.
// DRY_RUN=1 prints the rows it would write instead of calling the tracker.

const token = process.env.TRACKER_TOKEN;
const apiUrl = process.env.TRACKER_API_URL;
const apiHeaders = JSON.parse(process.env.TRACKER_API_HEADERS || '{}');
const tasks = process.env.TRACKER_TASKS_ID;
...
async function tracker(path, body, method = 'POST') {
  ...
  if (!res.ok) throw new Error(`tracker ${method} ${path} → ${res.status}: ${await res.text()}`);
  return res.json();
}
...
async function upsert(item, { relabel }) {
  ...
  const found = await tracker(`/data_sources/${tasks}/query`, {
  ...
  if (existing) {
    ...
    await tracker(`/pages/${existing.id}`, { properties: rowFor(item, { withType: relabel }) }, 'PATCH');
...
async function main() {
  if (!dryRun && (!token || !apiUrl || !tasks)) {
    throw new Error('TRACKER_TOKEN, TRACKER_API_URL and TRACKER_TASKS_ID are required');
  }
