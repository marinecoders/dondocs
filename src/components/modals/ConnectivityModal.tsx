  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-lg">
        ...
        {/* The run is held one level down so that it lives and dies with the
            dialog: closing unmounts this, so a token, an address and a verdict
            cannot outlast the session they belong to. Nothing to remember to
            clear, and no way to close without also ending the run. */}
        <ProbeForm />
      </DialogContent>
    </Dialog>
  );
...
function ProbeForm() {
  ...
  // Local state on purpose. A token typed here never reaches the store and
  ...
  // A request still out when the dialog goes is one nobody is waiting on, so
