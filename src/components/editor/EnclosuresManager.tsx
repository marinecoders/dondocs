            ...
            {enclosuresNotAllowed && (
              <div className="flex items-start gap-2 p-3 mb-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg text-sm">
                ...
                <div className="text-amber-800 dark:text-amber-200">
                  ...
                  {enclosures.length > 0 && (
                    <span className="block mt-1 text-xs">Your {enclosures.length} enclosure{enclosures.length !== 1 ? 's are' : ' is'} preserved and will reappear when you switch to a document type that uses them.</span>
                  )}
                </div>
              </div>
            )}
