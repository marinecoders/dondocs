...
% Custom \uline: underline at fixed depth so descenders cross through the line
% (like Word), instead of \underline which pushes the line below descenders
\makeatletter
\newcommand{\uline}[1]{%
  \leavevmode
  \setbox\@tempboxa\hbox{#1}%
  \rlap{\raisebox{-1.5pt}{\rule{\wd\@tempboxa}{0.4pt}}}%
  \usebox\@tempboxa
}
\makeatother
...
% Prevent widow/orphan lines per SECNAV M-5216.5 Ch 7 ¶13:
% "At least two lines of a paragraph must appear on any page"
\widowpenalty=10000
\clubpenalty=10000
...
%=============================================================================
% Variables defined here (before \input{document}) so document.tex can
% set them via \renewcommand. The format templates use these variables
% in their \printDateAndTitle commands.

\newcommand{\EndorsementOrdinal}{}      % FIRST, SECOND, THIRD, etc.
\newcommand{\EndorsementSerial}{}       % Ser XXX/### (same-page only)
\newcommand{\EndorsementDate}{}         % Date of endorsement (same-page only)
\newcommand{\BasicLetterID}{}           % e.g., "NAS Meridian ltr 5216 Ser 11/273 of 22 Apr 15"


%=============================================================================
%                    EXECUTIVE MEMO CONFIGURATION
%=============================================================================
% Variables defined here (before \input{document}) so document.tex can
% set them. The executive format templates use these in their print commands.

% Standard Memorandum + Action Memorandum
...
% Custom classification: sets banner markings only (no CUI/Classified info block)
...
% Underline is applied by the generator when the user enables it (via \uline{} in
% the argument), so \setSubject stores the content as-is without forcing underline.
\newcommand{\setSubject}[1]{\renewcommand{\SubjectLine}{#1}}

% Continuation page subject line (shown on page 2+ when enabled)
...
% Business/executive recipient address (pre-formatted with line breaks)
...
% Print "POC: <email>" as a clickable mailto link, OR nothing when no
% pocEmail has been set. Without the empty-check, the templates would
% emit a literal "POC: " followed by a zero-width \href{mailto:}{} link
% (issue #50) — a SECNAV no-no since the label "POC:" appears with no
% content, and the empty link still has a clickable hover/cursor zone.
%
% Used by every CUI / Classified block across all 14 templates that
% render a POC line (see tex/templates/*.tex). Single source of truth.
%
% Note: \ifNotEmpty is defined later in this file (line ~696) but is
% only resolved at expansion time, so this forward reference is safe.
\newcommand{\printPOCLine}{%
    \ifNotEmpty{\POCEmail}{POC: \href{mailto:\POCEmail}{\POCEmail}}%
}
...
\newcommand{\printLetterhead}{%
    ...
    \begin{textblock*}{4in}(2.25in, 0.625in)%
        ...
        % Department name conditional on \Department setting
        \ifthenelse{\equal{\Department}{navy}}{%
            \textcolor{\LetterheadColor}{\textbf{DEPARTMENT OF THE NAVY}}%
        }{%
            \ifthenelse{\equal{\Department}{dod}}{%
                \textcolor{\LetterheadColor}{\textbf{DEPARTMENT OF DEFENSE}}%
            }{%
    ...
    \end{textblock*}%
    % Spacing: Letterhead ends ~0.975in from top (0.625in start + ~0.35in content)
    % Text area starts at ~1.02in (top=1in + headheight=15pt + headsep=12pt)
    % 0.16in ≈ 12pt ≈ one line gap from letterhead bottom to SSIC block
    \vspace*{0.16in}%  % One line below letterhead area
}
...
%=============================================================================
% Per SECNAV M-5216.5 Ch 8: Distribution = action addressees
% The generator emits \setHasDistribution and \DistributionRows{...}.

\newif\ifhasdistribution
\hasdistributionfalse
...
%                        COPY TO COMMANDS
%=============================================================================
% Per SECNAV M-5216.5 Ch 7, Para 15: Copy to = information addressees
% The generator emits \setHasCopyTo and \CopyToRows{...}.
...
\newcommand{\parasection}[1]{%
    ...
    \vspace{12pt}%
    \noindent\arabic{mainpara}. \uline{#1}:%
}
...
%=============================================================================
% DISTRIBUTION - Action addressees (shows after signature if any defined)
%=============================================================================
...
%=============================================================================
% COPY TO - Information addressees (shows after distribution if any defined)
%=============================================================================
