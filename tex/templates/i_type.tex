...
% "If your document ends on an odd page the page number will be written
% 7/(8 blank)": the verso that print would leave blank is numbered on the
% recto before it. The last page is known at its own shipout: a mark set
% at the end of the document lands on it, and only it, so no page count
% from an earlier pass is needed. \nextPageLabel is set by each page style,
% since an appendix numbers its pages A-1, A-2.
\AtEndDocument{\markboth{DOCEND}{}}
\newcommand{\nextPageLabel}{\the\numexpr\value{page}+1\relax}
\newcommand{\appendixLetter}{A}
\makeatletter
\newcommand{\pubPageNumber}{%
    \def\@docend{DOCEND}\edef\@here{\leftmark}%
    \ifx\@here\@docend
        \ifodd\value{page}\thepage/(\nextPageLabel\ blank)\else\thepage\fi
    \else\thepage\fi}
\makeatother
...
\fancypagestyle{appendixpage}{%
    ...
    \renewcommand{\nextPageLabel}{\appendixLetter-\the\numexpr\value{page}+1\relax}%
    \fancyfoot[C]{\pubPageNumber}%
    \renewcommand{\headrulewidth}{0pt}%
    \renewcommand{\footrulewidth}{0pt}%
    \setlength{\headheight}{26pt}%
}
