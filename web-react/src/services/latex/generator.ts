...
export function generateSignatoryTex(store: DocumentStore): string {
  ...
  // Set signature image if present (will be written to attachments/signature.png)
  const signatureImageTex = data.signatureImage?.data
    ? '\\setSignatureImage{signature.png}'
    : '\\setSignatureImage{}';
